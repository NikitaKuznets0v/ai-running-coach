...
bot.on('message:document', async (ctx) => {
  ...
  try {
    ...
    if (errorMsg.includes('unsupported image') || errorMsg.includes('pdf')) {
      await ctx.reply('❌ К сожалению, PDF пока не поддерживается.\n\n💡 **Решение**: Сделай **скриншот страницы** с результатами теста и отправь как **фото** (не как файл).\n\nИли напиши данные текстом:\n• VO2max\n• LTHR (пульс на ПАНО)\n• HR зоны');
    } else {
