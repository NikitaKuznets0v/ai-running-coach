...
### Распознавание фото + Статистика

Теперь бот умеет распознавать фото тренировок и показывать статистику за любой период.

### Добавлено
- **Распознавание фото тренировок** через GPT-4o Vision:
  - Поддержка скриншотов из беговых приложений (Strava, Nike Run Club, Garmin Connect, etc.)
  - Поддержка фото с беговой дорожки
  - Автоматическое извлечение: дистанция, время, темп, пульс, калории, набор высоты
  - Сохранение с `source='screenshot'` в таблицу `trainings`
  - Уведомление о распознанных данных с эмодзи

- **Статистика за период**:
  - Запросы типа "как я побегал на прошлой неделе", "статистика за месяц"
  - Суммарная дистанция, количество тренировок, среднее время
  - Анализ прогресса от AI-тренера
  - Поддержка периодов: сегодня, вчера, неделя, месяц, произвольный

- **Автоматическое обновление user_stats**:
  - После каждой записанной тренировки (текст или фото)
  - Обновление полей: total_distance_km, total_trainings, total_duration_seconds
  - Недельная/месячная статистика: week_distance_km, month_trainings
  - Стрики: current_streak_days, longest_streak_days
  - last_training_date для отслеживания активности

- **Новый workflow v5** с параллельными ветками:
  - Ветка для фото: Download Photo → Prepare Image Data → GPT-4o Vision → Save → Update Stats → Response
  - Ветка для текста: сохранена логика v4 + stats intent detection
  - Загрузка последних 50 тренировок для контекста статистики

### Технические детали
- Workflow: `workflow-main-chatbot-v5.json`
- Деплой: n8n.kube.kontur.host
- ID воркфлоу: `7Ar459SadzSXgUEv`
- GPT-4o используется для Vision (распознавание изображений)
- GPT-4o-mini для текстовых запросов (экономия токенов)

### Удалено
- Workflow v4 (заменён на v5)
...
- [ ] Еженедельные сводки прогресса (автоматические)
- [ ] Ачивки и награды (5К, 10К, PB, стрики)
- [ ] Дашборды с графиками (будущее)
